...
function addCounselor() {
  const name = document.getElementById("namekons").value;
  const noHp = document.getElementById("nohpkons").value;
  const email = document.getElementById("email_signupkons").value;
  const password = document.getElementById("psw_signupkons").value;

  // Memasukkan data ke dalam koleksi "counselor"
  db.collection("counselor")
    .add({
      name: name,
      noHp: noHp,
    ...
    .then((docRef) => {
      console.log("Data counselor berhasil ditambahkan dengan ID: ", docRef.id);
      // Lakukan tindakan selanjutnya setelah data berhasil ditambahkan
    })
    .catch((error) => {
      console.error("Error saat menambahkan data counselor: ", error);
    });
}

// Menambahkan event listener untuk tombol "Buat Akun"
const signUpButton = document.getElementById("signup-buttonkons");
signUpButton.addEventListener("click", addCounselor);
